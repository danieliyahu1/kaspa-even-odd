...
test('recovery projection fails closed and logs only safe fields', () => {
  ...
  assert.deepEqual(entries[0], { transactionId: 'aa'.repeat(32), action: 'reveal', protocolVersion: 'EO/v4', templateHash: undefined, phase: 'joined', confirmationStatus: 'confirmed' });
...
test('recovery checkpoints authoritative state and reloads after a wallet change', async () => {
  ...
  assert.equal((await store.load('EO/v4\u0000recovery\u0000testnet-10\u0000' + 'aa'.repeat(32))).checkpoint.daaScore, '7');
  const events = [];
